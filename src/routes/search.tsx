...
function getCategoryShopHeading(categorySlug?: string): string {
  ...
  if (norm.includes("elec") || norm.includes("mobile"))
    return "🏪 Electronics & Tech Shops Near You";
  if (norm.includes("meat")) return "🏪 Meat & Fish Shops Near You";
  if (norm.includes("bakery") || norm.includes("sweet"))
    return "🏪 Bakeries & Sweet Shops Near You";
  if (norm.includes("home") || norm.includes("decor") || norm.includes("furniture"))
    return "🏪 Home & Decor Shops Near You";
...
function SwiggySearchPage() {
  ...
  const listingCandidates: MerchandisingProduct[] = products.map((product) => ({
    id: product.id,
    ...
    created_at: "",
    average_rating: Number(product.rating),
    review_count: Number(product.review_count),
    shop_name: product.shop_name,
  }));
  const localListingCandidates: MerchandisingProduct[] = Object.values(productsByStore)
    .flat()
    .map((product) => {
  ...
  const listingSourceProduct = {
    id: "search-context",
    seller_id: "search-context",
    name: query || filterState.category || "",
    brand: null,
    brand_id: null,
    brand_name: null,
  ...
  const shopHeading = hasConfirmedLocation
    ? getCategoryShopHeading(filterState.category)
    : "Recommended Shops";
    ...
    <AppShell>
      <div className="min-h-screen w-full min-w-0 max-w-7xl mx-auto overflow-hidden rounded-[28px] border border-[#eadff0] bg-[#fbf9fc] px-3 pb-20 pt-4 shadow-[0_16px_50px_rgba(76,29,149,0.06)] sm:px-6">
        ...
        {intent.category_intent && (
          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-[#700b6e]/20 border border-[#c026d3]/20 text-xs text-[#f0abfc] mb-4">
            <Sparkles className="w-4 h-4 text-[#c026d3] shrink-0" />
            <span>
              Detected intent:{" "}
              <strong className="font-semibold text-foreground">{intent.category_intent}</strong>
            </span>
          </div>
        )}
        ...
        <div className="mb-6 flex min-w-0 flex-col gap-3 rounded-2xl border border-[#eadff0] bg-white px-3 py-3 shadow-xs sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-primary">
              LocalShore catalog
            </p>
            <h1 className="font-display text-xl font-black text-foreground sm:text-2xl">
              {query
                ? `Results for "${query}"`
                : filterState.category
                  ? `Local Marketplace`
            ...
            <p className="text-xs text-muted-foreground font-semibold mt-0.5">
              ...
              {loading && (
                <span className="animate-pulse text-primary ml-2 font-extrabold">
                  • Updating...
                </span>
              )}
            </p>
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="sticky top-20 hidden w-64 shrink-0 rounded-2xl border border-[#eadff0] bg-white p-3 shadow-xs md:block lg:w-72">
            <DynamicFilterPanel
          ...
          <div className="flex-1 min-w-0 space-y-8">
            ...
            {/* ============================================================ */}
            <section
              className="space-y-4 rounded-2xl border border-[#eadff0] bg-white p-4 shadow-xs sm:p-5"
              aria-label={hasConfirmedLocation ? "Shops Near You" : "Recommended Shops"}
            >
            ...
            <section
              className="space-y-4 rounded-2xl border border-[#eadff0] bg-white p-4 shadow-xs sm:p-5"
              aria-label="Products From Local Shops"
            >
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg sm:text-xl font-extrabold text-foreground flex items-center gap-2">
              ...
              {loading ? (
                ...
                <div className="text-center py-12 bg-card border hairline rounded-2xl p-8">
                  ...
                  <p className="text-xs text-muted-foreground mt-1 max-w-md mx-auto">
                    Try broadening price range or clearing attribute filters to view available local
                    inventory.
                  </p>
                  <Button
                    type="button"
            ...
            <RelatedProductsSection
              sourceProduct={listingSourceProduct}
              candidates={[...listingCandidates, ...localListingCandidates]}
              queryLabel={query || filterState.category}
            />
          </div>
        </div>
