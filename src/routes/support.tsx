...
const supportStages = [
  ...
  "refund_initiated", "refunded", "replacement_approved", "replacement_delivered",
];
...
function SupportPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      await refresh();
      refreshListener = () => { void refresh(); };
      channel = supabase
        .channel(`customer-support-${auth.user.id}`)
        .on("postgres_changes", { event: "*", schema: "public", table: "support_tickets", filter: `user_id=eq.${auth.user.id}` }, refreshListener)
        .subscribe();
