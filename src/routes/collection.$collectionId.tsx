...
function CollectionPage() {
  ...
  const collection = useQuery({
    queryKey: ["collection", kind, collectionId],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from(kind === "gift" ? "gift_collections" : "seasonal_collections")
    ...
    <AppShell>
      <div className="px-5 py-6 md:px-8">
        ...
        <div className="mt-5 rounded-xl bg-card p-6 ring-1 ring-black/[0.05]">
          <h1 className="font-display text-3xl">{collection.data?.name ?? "Collection"}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {collection.data?.description ?? "Curated picks from approved local vendors."}
          </p>
        </div>
        <div className="mt-6 flex gap-3 overflow-x-auto pb-3">
          {(products.data as MerchandisingProduct[] | undefined)?.map((product) => (
