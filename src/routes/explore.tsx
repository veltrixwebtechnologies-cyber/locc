...
function ExplorePage() {
  ...
  const types: ("all" | TravelOpportunity["type"])[] = [
    "all",
    "culture",
    "retail",
    "business",
  ...
  const filtered =
    filter === "all" ? TRAVEL_OPPORTUNITIES : TRAVEL_OPPORTUNITIES.filter((t) => t.type === filter);
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="mt-2 max-w-xl text-sm text-sky-200">
              Explore the world's best local shopping cultures, markets, and retail experiences.
              Curated travel opportunities from our travel partners.
            </p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((dest, i) => (
            ...
            >
              ...
              <div className="p-4">
                <span
                  className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${typeLabel[dest.type].color}`}
                >
                ...
                <p className="mt-2 text-[11px] text-slate-400">via {dest.partner}</p>
              </div>
            </m.div>
          ))}
        </div>
