...
function ProductPage() {
  ...
  if (product.isLoading)
    return (
      <AppShell>
        <div className="grid gap-6 p-5 md:grid-cols-[minmax(260px,380px)_1fr] md:p-8">
          <SkeletonCard className="h-[380px]" />
          <div className="space-y-4">
            <div className="premium-skeleton h-4 w-1/3 rounded" />
            <div className="premium-skeleton h-10 w-4/5 rounded" />
            <div className="premium-skeleton h-5 w-1/2 rounded" />
  ...
  return (
    <AppShell>
      <div className="px-5 py-6 md:px-8">
        ...
        <div className="mt-5 grid gap-8 md:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]">
          ...
          <div>
            <div className="flex items-start justify-between gap-3">
              ...
              <WishlistButton
                ...
                item={{
                  ...
                  price: Number(item.discount_price ?? item.selling_price),
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              {item.category ?? "Product"} {item.brand_name ? `· ${item.brand_name}` : ""}
            </p>
            <div className="mt-5 flex items-center gap-3">
              ...
              <span className="inline-flex items-center gap-1 text-sm">
                <Star className="h-4 w-4 fill-[var(--marigold)] text-[var(--marigold)]" />{" "}
        ...
        <section className="mt-8 rounded-xl bg-card p-5 ring-1 ring-black/[0.05]">
          ...
          <div className="mt-6 space-y-3">
            {reviews.data?.map((review: any) => (
              <article key={review.id} className="border-t pt-3">
...
function ProductGallery({ item }: { item: MerchandisingProduct }) {
  const images = item.image_url ? [item.image_url] : [];
  const [selectedImage, setSelectedImage] = useState(images[0] ?? "");
  const activeImage = selectedImage || images[0] || undefined;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-[88px_minmax(0,1fr)] sm:items-start">
      <div className="order-2 flex gap-3 overflow-x-auto sm:order-1 sm:flex-col">
        {(images.length ? images : [""]).map((image, index) => (
          <button
            ...
            className={`h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-card p-1.5 transition-shadow ${
              activeImage === image
                ? "ring-2 ring-primary ring-offset-2"
                : "ring-1 ring-black/[0.06] hover:ring-primary/50"
            }`}
          >
            <ProductThumb
              src={image || undefined}
      ...
      <m.div
        ...
        className="relative flex min-h-[360px] items-center justify-center overflow-hidden rounded-2xl border border-[#ead9a8] bg-white p-6 shadow-sm sm:order-2 md:min-h-[500px]"
      >
        <ProductThumb
          src={activeImage}
          alt={item.name}
          ...
          fit="contain"
        />
        <span className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-background/90 px-3 py-1 text-[11px] text-muted-foreground shadow-sm">
          Product image
        </span>
      </m.div>
    </div>
