...
export function ShopStatusBadge({
  ...
  if (loading) {
    return (
      <span
        className={`inline-flex items-center rounded-full font-medium bg-muted text-muted-foreground ${s.wrap} ${className}`}
      >
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full font-semibold ${palette[kind] ?? "bg-muted text-muted-foreground"} ${s.wrap} ${className}`}
    >
...
export function ShopStatusCard({
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <div>
          <p className="text-sm font-semibold">Checking availability…</p>
  ...
  if (!status) {
    return (
      <div
        className={`flex items-center gap-3 rounded-xl border border-dashed border-muted bg-card/60 p-4 ${className}`}
      >
  ...
  const extra =
    kind === "holiday"
      ? overrideReason
      : kind === "closed_override"
        ? overrideReason || "Temporarily closed by owner"
        : kind === "open_override"
          ? overrideReason || "Manually opened by owner"
          : null;
  return (
...
export function ShopClosedOverlay({ status }: { status?: ShopStatus | null }) {
  ...
  const message =
    status.status === "holiday"
      ? `Closed · ${status.overrideReason ?? "Holiday"}`
      : status.status === "closed_override"
        ? "Temporarily Closed"
        : status.opensAt
