...
const issueGroups: Array<{ label: string; items: Array<{ id: IssueType; label: string }> }> = [
  {
    ...
    items: [
      ...
      { id: "quantity_missing", label: "Quantity missing" },
      { id: "not_fresh", label: "Item not fresh" },
  ...
  },
  {
    ...
    items: [
      { id: "delivered_late", label: "Order delivered late" },
  ...
  },
  {
    label: "Payment issue",
    items: [
      { id: "double_payment", label: "Double payment" },
      { id: "payment_failed", label: "Payment failed but money debited" },
...
const itemIssue = new Set<IssueType>([
  "wrong_item",
  "missing_item",
  "damaged_product",
  "expired_product",
...
const labelFor = (id: IssueType) =>
  issueGroups.flatMap((g) => g.items).find((i) => i.id === id)?.label ?? "Other issue";
...
export function OrderSupport({ order }: { order: Order }) {
  ...
  const chooseFiles = (list: FileList | null) => {
    ...
    if (incoming.some((file) => !file.type.startsWith("image/") && !file.type.startsWith("video/")))
      toast.error("Upload images or one short video only.");
  };
  ...
  const submit = async () => {
    if (
      !issue ||
      (needsItems && selectedItems.length === 0) ||
      (imageRequired.has(issue) && images.length === 0)
    )
      return;
      ...
      const uploaded = await Promise.all(
        files.map(async (file) => {
          ...
          const { error } = await withTimeout(
            supabase.storage.from("support-evidence").upload(path, file, { upsert: false }),
            "Evidence upload timed out. Check that the support-evidence bucket migration is applied.",
          );
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="mr-auto">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Our support team can review item-specific issues.
            </p>
          </div>
          <button
            type="button"
            onClick={() => setOpen(true)}
            className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-teal-deep"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border hairline px-3 py-2 text-sm hover:bg-muted"
          >
            <FileText className="h-4 w-4" /> Invoice
          </button>
          <Link to="/help" className="rounded-lg border hairline px-3 py-2 text-sm hover:bg-muted">
            Contact support
          </Link>
        </div>
      </section>
    ...
    <section className="mx-5 mt-4 mb-8 rounded-xl bg-card p-4 ring-1 ring-black/[0.04]">
      {submittedId ? (
        <div className="py-4 text-center">
          ...
          <h2 className="mt-2 font-display text-lg">Support request received</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Case <span className="font-mono">{submittedId.slice(0, 8).toUpperCase()}</span> is now
            under review.
          </p>
          <Link
            to="/support"
        ...
        <>
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 font-display text-lg">Report an order issue</h2>
            </div>
            <button
              type="button"
              aria-label="Close"
          ...
          {!withinWindow && (
            <div className="mt-3 flex gap-2 rounded-lg bg-amber-50 p-3 text-xs text-amber-900">
              <AlertCircle className="h-4 w-4 shrink-0" /> This order is outside the 48-hour
              reporting window.
            </div>
          )}
          {step === 1 && (
            <div className="mt-4">
              <p className="text-sm font-medium">Which items are affected?</p>
              <p className="mt-1 text-xs text-muted-foreground">
                Select one or more items, or continue for a delivery or payment issue.
              </p>
              <div className="mt-3 space-y-2">
                {order.lines.map((line) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-xs text-muted-foreground">{line.qty}×</span>
                  </label>
                ))}
              </div>
            </div>
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {group.items.map((item) => (
                      <button
                        ...
                        className={`rounded-lg border p-3 text-left text-sm transition-colors ${issue === item.id ? "border-primary bg-primary/10 font-medium" : "hairline hover:bg-muted"}`}
                      >
                        {item.label}
                      </button>
                    ))}
          ...
          {step === 3 && (
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-sm font-medium">Add details</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {itemNames.length ? itemNames.join(", ") : "Order-level issue"}
                </p>
              </div>
              <label className="block text-sm">
                ...
                <textarea
                  ...
                  rows={4}
                  className="mt-2 w-full rounded-lg border hairline bg-background p-3 text-sm outline-none focus:border-primary"
              ...
              <div>
                <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed p-4 text-sm hover:bg-muted">
                  <Upload className="h-4 w-4" /> Add photos or one short video
                  <input
                    type="file"
                    hidden
                    multiple
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  {imageRequired.has(issue!) ? " A photo is required for this issue." : ""}
                </p>
                {files.length > 0 && (
          ...
          <div className="mt-5 flex justify-between gap-2">
            <button
              ...
              disabled={submitting}
              onClick={() =>
                step === 1 ? (setOpen(false), reset()) : setStep((current) => current - 1)
              }
              className="inline-flex items-center gap-1 rounded-lg border hairline px-3 py-2 text-sm hover:bg-muted disabled:opacity-50"
            >
            ...
            {step < 3 ? (
              ...
              <button
                type="button"
                disabled={!canContinue || submitting || !withinWindow}
                onClick={() => void submit()}
