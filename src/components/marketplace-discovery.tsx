        ...
        <div className="grid lg:grid-cols-[220px_1fr_250px]">
          ...
          <div className="p-4 md:p-5">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-widest text-primary">Discover nearby</p>
                <h2 className="font-display text-lg font-bold text-foreground">Frequently searched</h2>
              </div>
          ...
          >
            <div>
              <p className="text-xs font-medium text-primary-foreground/80">Trusted sellers near you</p>
              <h3 className="mt-2 font-display text-2xl font-bold leading-tight">Discover local picks</h3>
              <p className="mt-2 text-xs leading-5 text-primary-foreground/80">Fresh products, quick delivery, and neighborhood stores you can trust.</p>
            </div>
            <span className="mt-5 inline-flex w-fit rounded-full bg-background px-4 py-2 text-xs font-bold text-foreground">View products</span>
          </Link>
        </div>
      </div>
      <SocialShoppingStrip />
  ...
  return (
    ...
    >
      {signals.map(({ icon: Icon, label, detail }) => (
        <div key={label} className="flex min-w-[220px] items-center gap-2.5 rounded-full border border-[#ead9a8] bg-card px-3 py-2">
  ...
  return (
    ...
    >
      <div className="flex aspect-[1.35] items-center justify-center bg-[#f7f7f7] p-3">
        {imageUrl ? (
          <img src={imageUrl} alt={product?.name ?? item.title} loading="lazy" className="h-full w-full object-contain mix-blend-multiply transition-transform group-hover:scale-105" />
        ) : (
      ...
      <div className="p-2.5">
        ...
        <p className="mt-1 text-[10px] text-muted-foreground">Shop now <span className="text-primary">›</span></p>
      </div>
    </Link>
  );
}
